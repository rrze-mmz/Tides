@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}"
         class="flex items-center justify-between d">
        <div class="flex justify-between flex-1 sm:hidden">
            @if ($paginator->onFirstPage())
                <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 bg-white dark:bg-gray-900 dark:text-white
            ...
            @else
                ...
                text-sm font-medium text-gray-700 bg-white dark:bg-gray-900 dark:text-white border border-gray-300 leading-5 rounded-md
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" class="relative inline-flex items-center px-4
                py-2 ml-3 text-sm font-medium text-gray-700 bg-white dark:bg-gray-900 dark:text-white border border-gray-300 leading-5
                rounded-md hover:text-gray-500 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300
                active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
                    {!! __('pagination.next') !!}
                </a>
            @else
                <span class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium
                text-gray-500 bg-white dark:bg-gray-900 dark:text-white border border-gray-300 cursor-default leading-5 rounded-md">
                    {!! __('pagination.next') !!}
                </span>
            @endif
        </div>
                ...
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    ...
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                            <span
                                ...
                                text-gray-500 bg-white dark:bg-gray-900 dark:text-white border border-gray-300 cursor-default rounded-l-md leading-5"
                                aria-hidden="true">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                           ...
                           bg-white dark:bg-gray-900 dark:text-white border border-gray-300 rounded-l-md leading-5 hover:text-gray-400 focus:z-10
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span aria-disabled="true">
                                <span
                                    class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white dark:bg-gray-900 dark:text-white border border-gray-300 cursor-default leading-5">{{ $element }}</span>
                            </span>
                        @endif
                                ...
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span
                                            ...
                                            font-medium text-gray-500 bg-white dark:bg-gray-900 dark:text-white border border-gray-300 cursor-default
                                            leading-5">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}"
                    ...
                    @else
                        <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                            <span
                                ...
                                text-gray-500 bg-white dark:bg-gray-900 dark:text-white border border-gray-300 cursor-default rounded-r-md leading-5"
                                aria-hidden="true">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
